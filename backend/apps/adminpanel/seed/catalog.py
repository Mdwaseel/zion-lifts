...
from ..models import Application, Component, Finish, Lift, SafetyFeature
...
def run():
    """Write the catalogue. Idempotent — keyed on slug, so re-running refreshes."""
    apps_by_slug = {}
    for i, (slug, name, group, desc) in enumerate(APPLICATIONS, 1):
        obj, _ = Application.objects.update_or_create(
            slug=slug,
            defaults=dict(name=name, group=group, description=desc, order=i),
        )
    ...
    for row in LIFTS:
        ...
        # The three lists that used to be three tables. They are written as part
        # of the lift now, which is why this loop no longer deletes children
        # before re-creating them: assigning the column replaces the lot.
        row["images"] = [
            {"kind": kind, "src": url, "alt": alt, "caption": alt}
            for kind, url, alt in row.pop("images", [])
        ]
        row["variants"] = [
            {"code": code, "name": name, "description": desc, "capacity": cap,
             "persons": persons, "speed": speed, "shaft": shaft}
            for code, name, desc, cap, persons, speed, shaft in row.pop("variants", [])
        ]
        row["specs"] = [
            {"group": group, "label": label, "value": value, "note": note}
            for group, label, value, note in row.pop("specs", [])
        ]
        row["hero_image_url"] = row.pop("hero", "")
        row["hero_video_url"] = row.pop("hero_video", "")

        lift, _ = Lift.objects.update_or_create(slug=slug, defaults=row)
        lift.applications.set([apps_by_slug[s] for s in app_slugs if s in apps_by_slug])
        lift.safety_features.set(
            [safety_by_slug[s] for s in safety_slugs if s in safety_by_slug]
        )

    return {
        "applications": Application.objects.count(),
        "safety_features": SafetyFeature.objects.count(),
        "finishes": Finish.objects.count(),
        "components": Component.objects.count(),
        "lifts": Lift.objects.count(),
    }
