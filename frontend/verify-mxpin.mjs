...
async function run(label, opts) {
  ...
  const name = () => p.evaluate(() => document.querySelector('.mx__name')?.textContent)
  ...
  const after = await p.evaluate(() => {
    const pin = document.querySelector('.mx__pin').getBoundingClientRect()
    const proof = document.querySelector('.mx__cta').getBoundingClientRect()
...
await p.goto('http://localhost:5173/', { waitUntil: 'domcontentloaded' })
await p.waitForTimeout(1800)
console.log('tablet 768: pinned =', await p.evaluate(() => document.querySelector('.mx__scroller').classList.contains('is-pinned')))
await p.close()
