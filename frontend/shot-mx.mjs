...
const OUT = process.env.SHOT_DIR ?? 'C:/Users/Mohd/AppData/Local/Temp/claude/d--Projects-Zion-Lifts/7ccdca15-a9ce-4d60-bc16-08f19b29d5bf/scratchpad/mx'
...
for (const w of (widths.length ? widths : [1440])) {
  ...
  const runway = Math.max(0, sc.h - sc.vh)
  await p.evaluate((y) => scrollTo(0, y), Math.round(sc.top + runway * 0.28))
  await p.waitForTimeout(900)
  await p.screenshot({ path: path.join(OUT, `mx-${w}-a.png`) })
  await p.evaluate((y) => scrollTo(0, y), Math.round(sc.top + runway * 0.72))
  await p.waitForTimeout(700)
  await p.screenshot({ path: path.join(OUT, `mx-${w}-b.png`) })
